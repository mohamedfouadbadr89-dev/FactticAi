  ...
  return (
    <div className="min-h-screen bg-[var(--bg-primary)] text-[var(--text-primary)] p-10 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-10">
        {[
          { label: 'Risk Latency (P95)', value: effectiveData ? `${effectiveData.risk_latency.p95_ms}ms` : '—', icon: Clock, color: 'text-blue-400', href: '/dashboard/forensics' },
          { label: 'Drift Propagation', value: effectiveData ? effectiveData.drift_propagation.correlation_coefficient.toFixed(2) : '—', icon: Network, color: 'text-fuchsia-400', href: '/dashboard/behavior-forensics' },
          { label: 'Daily Alert Volume', value: effectiveData?.alert_frequency.total ?? '—', icon: ShieldAlert, color: 'text-amber-400', href: '/dashboard/alerts' },
          { label: 'Spike Incidents', value: effectiveData?.drift_propagation.spike_incidents ?? '—', icon: Zap, color: 'text-red-400', href: '/dashboard/behavior-forensics' }
        ].map((stat, i) => (
          <div key={i} onClick={() => router.push(stat.href)} className="bg-[var(--bg-secondary)] border border-[var(--border-primary)] p-6 rounded-3xl group hover:border-[var(--accent)]/30 transition-all cursor-pointer">
            ...
            <p className="text-[10px] font-black uppercase text-[var(--text-secondary)] tracking-widest mb-1">{stat.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-[2.5rem] p-10 h-[450px] flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-3 custom-scrollbar">
            {liveEvents.length === 0 ? (
               <div className="h-full flex flex-col items-center justify-center text-[var(--text-secondary)] border border-[var(--border-primary)] border-dashed rounded-xl">
              ...
              liveEvents.map((evt, i) => (
                <motion.div 
                  ...
                  className="bg-white/[0.02] border border-[var(--border-primary)] p-4 rounded-xl flex items-center justify-between"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-xs font-mono text-[var(--text-primary)]">Session: {evt.session_id?.slice(0,8)}</p>
                      <p className="text-[10px] text-[var(--text-secondary)] mt-1 uppercase tracking-widest">
                  ...
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <p className="text-[10px] font-black tracking-widest text-[var(--text-secondary)] uppercase">Risk</p>
                      <p className={`text-sm font-bold ${evt.risk_score >= 80 ? 'text-red-400' : evt.risk_score >= 60 ? 'text-amber-400' : 'text-emerald-400'}`}>
                        {evt.risk_score}
                      </p>
                    </div>
                    <div className="text-right w-16">
                      <p className="text-[10px] font-black tracking-widest text-[var(--text-secondary)] uppercase">Decision</p>
        ...
        <div className="lg:col-span-4 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-[2.5rem] p-10 flex flex-col">
          <h3 className="text-xs font-black uppercase tracking-widest text-[var(--text-secondary)] mb-8 flex items-center gap-2">
            ...
            {Object.entries(effectiveData?.alert_frequency.by_type ?? {}).map(([type, count], i) => {
              ...
              <div key={i}>
                <div className="flex justify-between items-end mb-2">
                  <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-primary)]">{type.replace(/_/g, ' ')}</span>
      ...
      <div className="mt-10 bg-[var(--bg-secondary)] border border-[var(--border-primary)] rounded-[2.5rem] p-10">
        <div className="flex justify-between items-center mb-10">
          ...
          <div className="flex gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-[var(--text-secondary)] uppercase">Operational</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-amber-500" />
              <span className="text-[10px] font-bold text-[var(--text-secondary)] uppercase">Degraded</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-red-500" />
              <span className="text-[10px] font-bold text-[var(--text-secondary)] uppercase">Critical</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {Object.entries(providerHealths).length === 0 ? (
            <div className="col-span-full py-12 text-center border border-[var(--border-primary)] border-dashed rounded-3xl">
              <p className="text-[var(--text-secondary)] text-xs font-medium italic">No active telemetry streams discovered.</p>
            </div>
             ...
             <motion.div 
               ...
               className="bg-white/[0.02] border border-[var(--border-primary)] p-5 rounded-[1.5rem] relative overflow-hidden"
             >
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-[9px] font-mono text-[var(--text-secondary)] tracking-tighter">{health.latency}ms</span>
                </div>
                <h4 className="text-xs font-black uppercase tracking-tight text-[var(--text-primary)]">
                  {id.slice(0, 8)}...
                </h4>
                <p className="text-[10px] text-[var(--text-secondary)] mt-1 font-medium">{health.status}</p>
             </motion.div>
          ))}
        </div>
      </div>
      ...
      <footer className="mt-10 bg-white/[0.02] border border-[var(--border-primary)] p-8 rounded-[2rem] flex items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          <div>
            ...
            <p className="text-[10px] font-medium text-[var(--text-secondary)]">Cross-engine propagation analysis verified via institutional auditors.</p>
          </div>
        </div>
