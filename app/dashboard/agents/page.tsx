    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white p-8">
      ...
      <div className="flex items-center justify-between mb-12">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-[#1a1a1a] rounded-xl border border-[#2d2d2d] flex items-center justify-center shadow-2xl">
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            disabled={seeding}
            className="flex items-center gap-2 px-4 py-2 bg-[#111] border border-[#2d2d2d] hover:border-[#3b82f6] text-[#9ca3af] hover:text-[#3b82f6] rounded-lg text-[10px] font-black uppercase tracking-widest transition-all disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {AGENT_STATS.map((stat) => (
          <div key={stat.name} className="bg-[#111] border border-[#2d2d2d] rounded-2xl p-6 relative overflow-hidden group hover:border-[#444] transition-all">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest text-[#555] mb-1">{stat.name}</p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 mb-12">
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className="bg-[#111] border border-[#2d2d2d] rounded-2xl p-8">
            <div className="flex items-center justify-between mb-8">
              <h2 className="text-[11px] font-black uppercase tracking-widest text-[#555] flex items-center gap-2">
                <LayoutGrid className="w-3 h-3 text-[#3b82f6]" /> Managed Agent Sessions
              </h2>
              <span className="text-[9px] font-mono text-[#444] uppercase tracking-tighter">Live Sync: ACTIVE</span>
            </div>

            <div className="space-y-4">
              {loading && !sessions.length ? (
                <div className="h-64 flex items-center justify-center animate-pulse text-[#333]">
                   <Terminal className="w-8 h-8 animate-spin" />
                </div>
              ) : sessions.length === 0 ? (
                <div className="h-48 border border-dashed border-[#2d2d2d] rounded-xl flex flex-col items-center justify-center text-[#444]">
                ...
                <div key={s.id} className="bg-[#0a0a0a] border border-[#2d2d2d] rounded-xl p-5 group hover:border-[#3b82f644] transition-all">
                   <div className="flex items-center justify-between">
                      <div className="flex items-center gap-5">
                         <div className="w-12 h-12 rounded-lg bg-[#111] border border-[#2d2d2d] flex items-center justify-center">
                            <Bot className="w-6 h-6 text-[#3b82f6]" />
                         </div>
                         <div>
                            <div className="flex items-center gap-2">
                               <h3 className="text-sm font-black text-white">{s.agent_name}</h3>
                               <div className="px-2 py-0.5 rounded text-[8px] font-black bg-[#1a1a1a] border border-[#2d2d2d] text-[#555] uppercase font-mono">
                                  {s.session_id}
                               </div>
                            </div>
                            <div className="flex items-center gap-4 mt-2">
                               <div className="flex items-center gap-1.5 text-[9px] font-black text-[#555] uppercase">
                      ...
                      <div className="flex items-center gap-4">
                         <div className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border ${
                           ...
                           'bg-[#1a1a1a] text-[#555] border-[#2d2d2d]'
                         }`}>
                           {s.status}
                         </div>
                         <div className="flex items-center gap-2">
                            <button className="p-2 bg-[#111] border border-[#2d2d2d] hover:border-[#3b82f6] rounded-lg transition-colors group">
                               <Play className="w-3 h-3 text-[#555] group-hover:text-[#3b82f6]" />
                            </button>
                            <button className="p-2 bg-[#111] border border-[#2d2d2d] hover:border-[#ef4444] rounded-lg transition-colors group">
                               <ShieldX className="w-3 h-3 text-[#555] group-hover:text-[#ef4444]" />
                            </button>
                         </div>
                      </div>
                   </div>
                 ...
                 {[
                   ...
                   <div key={tool.name} className="flex flex-col gap-2">
                      <div className="flex justify-between items-center text-[10px] font-black uppercase">
                         ...
                         <span className="text-[#555] font-mono">{tool.usage} calls</span>
                      </div>
                      <div className="h-1 bg-[#0a0a0a] rounded-full overflow-hidden border border-[#1a1a1a]">
