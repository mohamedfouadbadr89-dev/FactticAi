...
ADD COLUMN IF NOT EXISTS system_mode TEXT NOT NULL DEFAULT 'bootstrap' CHECK (system_mode IN ('bootstrap', 'operational')),
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Drift Alerts: Viewable by org members') THEN
...
BEGIN
    ...
    IF v_dfi > 0.3 AND NOT EXISTS (
        ...
        INSERT INTO drift_alerts (org_id, severity, triggered_by, description)
