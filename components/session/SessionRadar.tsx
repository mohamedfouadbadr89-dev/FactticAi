...
export const SessionRadar: React.FC<SessionRadarProps> = ({ factors }) => {
  ...
  if (categories.length < 3) {
    return (
      <div className="flex flex-col items-center justify-center h-full border border-zinc-800 rounded-lg bg-zinc-950/30 p-8 text-center">
        ...
        <span className="text-[10px] font-mono font-bold uppercase tracking-widest text-zinc-600">Insufficient Data for Profiling</span>
        <p className="text-[9px] text-zinc-700 mt-1 max-w-[150px]">Requires at least 3 unique risk factors to generate signature.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="relative">
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="overflow-visible">
          ...
          {points.map((p, i) => (
            <text 
              ...
              y={p.labelY} 
              textAnchor="middle" 
              className="text-[9px] font-mono font-bold fill-zinc-500 uppercase tracking-tighter"
            >
              {categories[i]}
            </text>
          ))}
        ...
        {/* Legend / Info Overlay */}
        <div className="absolute top-0 right-0 p-2 text-[10px] font-mono text-zinc-600 bg-zinc-900/50 rounded border border-zinc-800">
            SIGNATURE_V1
        </div>
      </div>
      ...
      <div className="mt-4 flex gap-4">
        {categories.map((cat, i) => (
           <div key={i} className="flex items-center gap-1.5">
             ...
             <span className="text-[9px] font-mono text-zinc-400">{(data[i] * 10).toFixed(1)}</span>
           </div>
        ))}
      </div>
    </div>
